...
class TubeHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            ...
            if m:
                ...
                resultado = sorted(resultado, key=lambda v: v.get("fecha", 0), reverse=True)
                udata_canal = cargar_usuarios()
                info_canal = udata_canal.get("usuarios", {}).get(nombre_canal, {})
                self.enviar_json({
                    "autor": nombre_canal,
                    "num_videos": len(resultado),
                    "total_vistas": total_vistas,
                    "total_likes": total_likes,
                    "suscriptores": len(info_canal.get("suscriptores", [])),
                    "bio": info_canal.get("bio", ""),
                    "videos": resultado,
                })
                return
            ...
            if ruta == "/api/suscripciones/feed":
                ...
                for v in datos_v.get("videos", []):
                    if v.get("autor") in subs:
                        # visibilidad check
                        if v.get("visibilidad","publico") == "privado":
                            # include if user is subscriber or owner (it is subscriber)
                            pass
            ...
            # tendencias: top vídeos públicos de los últimos 7 días
            if ruta == "/api/tendencias":
                ...
                candidatos = [
                    ...
                    if v.get("visibilidad", "publico") == "publico"
                    and ahora - v.get("fecha", 0) < semana
                ]
                candidatos.sort(key=lambda v: v.get("vistas", 0) + v.get("likes", 0) * 3, reverse=True)
                self.enviar_json(candidatos[:50]); return
            ...
            if ruta == "/api/stats":
                ...
                total_bytes = sum(
                    (VIDEOS_DIR / v["archivo"]).stat().st_size
                    for v in datos.get("videos", [])
                    if (VIDEOS_DIR / v["archivo"]).exists()
                )
                self.enviar_json({
                    "videos": len(datos.get("videos", [])),
                    "usuarios": len(usuarios.get("usuarios", {})),
                    "almacenamiento_mb": round(total_bytes / 1024 / 1024, 2),
                    "reportes_pendientes": len(datos.get("reportes", [])),
                }); return

            # posición guardada para un vídeo concreto
            m = re.match(r'^/api/historial/posicion/([a-zA-Z0-9]+)$', ruta)
            if m:
                video_id = m.group(1)
                usuario = self.usuario_autenticado()
                if not usuario:
                    self.enviar_json({"posicion": 0}); return
                datos = cargar_usuarios()
                us = datos.get("usuarios", {}).get(usuario, {})
                posicion = 0
                for h in reversed(us.get("historial", [])):
                    if h.get("video") == video_id and h.get("posicion", 0) > 5:
                        posicion = h.get("posicion", 0)
                        break
                self.enviar_json({"posicion": posicion}); return
    ...
    def _video_publico(video):
        ...
        # we ensure tags and visibilidad exist
        v = {k: v for k, v in video.items() if k != "token"}
    ...
    def do_POST(self):
        ...
        try:
            ...
            if m:
                ...
                padre = (payload.get("padre") or "").strip()[:12] or None
                comentario = {"id": uuid.uuid4().hex[:12], "apodo": usuario, "texto": texto, "fecha": time.time(), "padre": padre}
            ...
            if ruta == "/api/suscripcion":
                ...
                if canal not in usuarios:
                    self.enviar_error_json("El canal no existe.", 404); return
                us = usuarios.setdefault(usuario, {})
                us.setdefault("suscripciones", [])
                target = usuarios[canal]
    ...
    def manejar_subida_video(self):
        ...
        for u_name, u_info in usuarios.get("usuarios", {}).items():
            if nuevo_video["autor"] in u_info.get("suscripciones", []):
                ...
                u_info.setdefault("notificaciones", []).append(noti)
    ...
    def do_DELETE(self):
        ...
        try:
            ...
            if m:
                ...
                datos["comentarios"][video_id] = [
                    c for c in lista
                    if c["id"] != comentario_id and c.get("padre") != comentario_id
                ]
                guardar_datos(datos)
                self.enviar_json({"ok": True}); return
            self.enviar_error_json("Ruta no encontrada.", 404)
        ...
        try:
            ...
            if m:
                ...
                if not video:
                    self.enviar_error_json("Vídeo no encontrado.", 404); return
                if video.get("autor") != usuario:
                    self.enviar_error_json("Solo el autor puede editar este vídeo.", 403); return
                titulo = (payload.get("titulo") or "").strip()[:100]
                if not titulo:
                    self.enviar_error_json("El título es obligatorio.", 400); return
                video["titulo"] = titulo
                video["descripcion"] = (payload.get("descripcion") or "").strip()[:500]
                cat = (payload.get("categoria") or video.get("categoria", "Otros")).strip()
                video["categoria"] = cat if cat else "Otros"
                tags_raw = (payload.get("tags") or "").strip()
                video["tags"] = [t.strip() for t in tags_raw.split(",") if t.strip()] if tags_raw else []
                vis = (payload.get("visibilidad") or "publico").strip()
                video["visibilidad"] = "privado" if vis == "privado" else "publico"
                guardar_datos(datos)
                self.enviar_json(self._video_publico(video)); return

            if ruta == "/api/perfil/bio":
                usuario = self.usuario_autenticado()
                if not usuario:
                    self.enviar_error_json("Debes iniciar sesión.", 401); return
                payload = self.leer_json_body()
                ...
                us = datos.get("usuarios", {}).get(usuario)
                if not us:
                    self.enviar_error_json("Usuario no encontrado.", 404); return
                us["bio"] = bio
                guardar_usuarios(datos)
                self.enviar_json({"bio": bio}); return
